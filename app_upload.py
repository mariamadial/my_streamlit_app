...
# un toggle switch
#dark_mode = st_toggle_switch(label="Mode sombre",key="dark_mode",default_value=False,label_after=False)
...
uploaded_file = st.file_uploader("Chargez un fichier CSV contenant les données des billets", type=["csv"])
...
if uploaded_file:
    ...
    if not all(col in data.columns for col in colonnes_attendues):
        ...
        # affichez les resultats à deux chffres apres la virgules
       ...
       # Histogrammeprediction_counts = results_df["Prédiction"].replace({0: "Faux billet", 1: "Vrai billet"}).value_counts().reset_index()prediction_counts.columns = ["Type de billet", "Nombre"]

# Création du graphique avec Seaborn
#sns.set_theme(style="whitegrid")  # Optionnel : ajuste le style du graphique
#fig = sns.barplot(data=prediction_counts, x="Type de billet", y="Nombre", palette="pastel").get_figure()

# Afficher le graphique directement dans Streamlit
#st.write("Distribution des Prédictions en graphiques:")
#st.pyplot(fig)
